...
export const riverBend: Scene = {
  ...
  render({ width: W, height: H, seed, coherence, rng }: SceneRenderParams): string {
    const palette = distortPalette(
      ['#4a6a30', '#5a7a40', '#3a7090', '#6ab0d0', '#c0e8f8'],
      coherence, rng
    )
    ...
    const riverWidth = jitter(W * 0.2, coherence, rng, W * 0.05)

    // S-curve control: enter top-right, exit bottom-left
    const startX = jitter(W * 0.7, coherence, rng, W * 0.1)
    const midX = jitter(W * 0.5, coherence, rng, W * 0.1)
    const endX = jitter(W * 0.3, coherence, rng, W * 0.1)
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      // Simple S via sin
      const x = startX + (endX - startX) * t + Math.sin(t * Math.PI) * (midX - startX) * jitter(1.0, coherence, rng, 0.3)
      const y = t * H
    ...
    const shimmerCount = Math.floor(jitter(8, coherence, rng, 4))
    for (let i = 0; i < shimmerCount; i++) {
      const t = rng()
      const p = centerPoints[Math.floor(t * (centerPoints.length - 1))]
      const offset = (rng() - 0.5) * riverWidth * 0.7
      const shimmerLen = jitter(30, coherence, rng, 20)
      const sx = p.x + offset
      const sy = p.y
    ...
    const highlightOpacity = (coherence * 0.4).toFixed(2)
