...
export const rainWindow: Scene = {
  ...
  render({ width: W, height: H, seed, rng }: SceneRenderParams): string {
    const palette = ['#2a3a4a', '#3a5060', '#6080a0', '#8090b0', '#c0d0e0']
    ...
    const rainCount = Math.floor(25)
    for (let i = 0; i < rainCount; i++) {
      const rx = rng() * W
      const startY = rng() * H * 0.3
      const endY = startY + H * 0.5
    ...
    const glareX = W * 0.3
    const glareOpacity = (0.15).toFixed(2)
