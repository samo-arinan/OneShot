...
export const kaleidoscope: Scene = {
  ...
  render({ width: W, height: H, seed, rng }: SceneRenderParams): string {
    const palette = ['#1A001A', '#6B0F6B', '#C050C0', '#FF99FF', '#FFE0FF']
    ...
    for (let layer = 0; layer < petalLayers; layer++) {
      const layerR = maxR * (1 - layer * 0.28)
      const innerR = layerR * 0.35
      const colorIdx = layer % palette.length

      for (let i = 0; i < symmetry; i++) {
        const baseAngle = (i / symmetry) * Math.PI * 2
        const angle = baseAngle
        const halfSpan = Math.PI / symmetry * 0.8
        ...
        const opacity = (0.45 + layer * 0.1 + 0.2).toFixed(2)
        petals.push(
