...
export const lighthouse: Scene = {
  ...
  render({ width: W, height: H, seed, rng }: SceneRenderParams): string {
    const palette = ['#1C3557', '#0D1B2A', '#E8E0D0', '#B8C8D8', '#FFE066']
    ...
    const towerBaseX = W * 0.68
    const towerBaseY = horizonY + H * 0.02
    const towerBaseW = W * 0.07
    const towerH = H * 0.48
    const towerTopW = W * 0.045
    const towerTopY = towerBaseY - towerH

    // Lantern room at top
    const lanternH = H * 0.07
    const lanternW = towerTopW * 1.4
    ...
    const beamCount = 5
    const beamAngleBase = -0.6
    const beamSpread = 0.8
