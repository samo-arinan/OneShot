...
export const stoneCircle: Scene = {
  ...
  render({ width: W, height: H, seed, rng }: SceneRenderParams): string {
    const palette = ['#4A4035', '#2A2018', '#7A6E5A', '#C8BEA8', '#E8DFC0']
    ...
    for (let i = 0; i < stoneCount; i++) {
      ...
      const stoneBaseY = horizonY + H * 0.02
      const stoneH = H * 0.22
      const stoneW = W * 0.045
