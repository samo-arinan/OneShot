...
export const bridgeArch: Scene = {
  ...
  render({ width: W, height: H, seed, rng }: SceneRenderParams): string {
    const palette = ['#4A6FA5', '#2C3E50', '#7F8C8D', '#BDC3C7', '#F39C12']
    ...
    const archCx = W * 0.5
    const archCy = horizonY
    const archRx = W * 0.3
    const archRy = H * 0.22
    const archThickness = H * 0.04

    // Bridge deck
    const deckY = horizonY
    const deckH = H * 0.03

    // Layer 3: Water reflection
    const waterOpacity = (0.7).toFixed(2)
    const reflectionScale = 0.6

    // Layer 4: Sky glow accent
    const glowX = archCx
    const glowY = archCy - archRy * 0.6
